...
import { USERCONTRACT } from "./config/userContractType";
...
export const addTenant = async (userData: any, adminId?: number) => {
  const {
    firstname,
    lastname,
    phone,
  ...
  try {
    ...
    `, [firstname, lastname, phone, invite_code, id_keycard, emergency_phone || '']);
    ...
    await connection.query(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'ACTIVE', ?)
    `, [
      contract_no, identity_card, address, room_id, 
      userId, rate_id, start_date, end_date, deposit, adminId
    ]);

    // 3. Update Room Status
    await connection.query('UPDATE Room SET room_status = "OCCUPIED" WHERE room_id = ?', [room_id]);
